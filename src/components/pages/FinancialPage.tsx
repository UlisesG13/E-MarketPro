...
import { projectPhases, budgetSummary, competitors, costByRole } from '../../services/mockData';
...
const FinancialPage: React.FC = () => {
  const totalCost = projectPhases.reduce(
    (sum, phase) => sum + phase.activities.reduce((s, a) => s + a.cost, 0), 0
  );
  ...
  const phaseCostData = projectPhases.map((p) => ({
    phase: p.name,
    cost: p.activities.reduce((s, a) => s + a.cost, 0),
            ...
            <tbody>
              {projectPhases.map((phase) =>
                phase.activities.map((activity, ai) => (
                  <tr key={`${phase.name}-${ai}`} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 text-white font-medium text-right">{formatCurrency(activity.cost)}</td>
                  </tr>
                ))
              )}
              {/* Total row */}
              <tr className="bg-indigo-500/10 border-t-2 border-indigo-500/30">
                ...
                <td className="p-4 text-indigo-300 text-right hidden sm:table-cell font-medium">
                  {projectPhases.reduce((s, p) => s + p.activities.reduce((sa, a) => sa + a.hours, 0), 0)} hrs
                </td>
