          ...
          {teamMembers.map((member, i) => (
            ...
            >
              ...
              <div className="text-right shrink-0">
                <p className="text-lg font-bold text-[var(--app-text)]">{formatCurrency((member.hourlyRate ?? 0) ?? 0)}</p>
            ...
            <tbody>
              {teamMembers.map((m) => (
                <tr key={m.id} className="border-b border-[var(--app-border)] transition-colors hover:bg-[var(--app-surface-soft)]">
                  ...
                  <td className="p-4 text-right text-[var(--app-text)]">{formatCurrency(((m.hourlyRate ?? 0) ?? 0))}</td>
                  <td className="p-4 text-right text-[var(--app-text-muted)]">{formatCurrency(((m.hourlyRate ?? 0) ?? 0) * 8)}</td>
                  <td className="p-4 text-right font-semibold text-[var(--app-text)]">{formatCurrency(((m.hourlyRate ?? 0) ?? 0) * 160)}</td>
                </tr>
              ))}
            </tbody>
          </table>
